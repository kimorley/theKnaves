...
#-------------------------------------------------------------------------------------------
cluster.saveCNVdata <- function(data, CHR, outDIR=getwd()){
	if (missing(data)){
	...
	a = unlist(strsplit(row.names(lrrData),"\\."))[seq(1,length(row.names(lrrData))*2,2)]	# Remove "X" prefix from non-rsID SNPs
	row.names(lrrData) = ifelse(substr(a,1,1) == "X", substr(a,2,nchar(a)), a)
	# Select BAF
