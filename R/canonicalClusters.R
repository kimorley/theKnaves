...
# lapply(clusterData, convertLines, gapiIntu=<FILENAME>, writeDir=<OUTDIR>)
# N.B. Memory requirements scale with size of GTU file
# Author: km5
###############################################################################

convertLines <- function(gapiGtu, gapiIntu, writeDir=getwd(), splitIDs=TRUE, returnData=FALSE){
	...
	if (sum(namesA==namesB)==length(row.names(INT))/2){
		...
		if (sum(row.names(GTU) == namesA) == length(namesA)){
			...
			gsCluster <- clusterSnp(results, mySnp, DIR=writeDir)
			if (returnData){
...
clusterSnp <- function(x, mySnp, DIR=getwd(), confThresh = 0.99, clusterStat = "median", saveCluster=TRUE, returnCluster=FALSE){
	# Check
	...
	summary <- tapply(newX$t,as.character(newX$call),clusterStat)
		...
		if (length(summary)==3){
			...
			print(paste("Number of genotypes should be between 1 and 3! Cannot generate canonical clusters for ",mySnp,".",sep=""))
			model <- 0
			summary <- 0
			clusterData <- list(model=model,summary=summary)
			if (saveCluster){
