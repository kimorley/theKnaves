calcLRR <- function(data, cluster="GS"){
	...
	# Check
	# Functions
	if (sum(names(data) %in% names(GS$model))==length(GS$model)){
		for (i in 1:length(data)){
			
			newX <- subset(data[[i]], select=c(t))				# Theta values for target samples
			model <- GS$model[[which(names(GS$model) == names(data)[i])]]	# Canonical cluster model for this SNP
			sum <- GS$sum[[which(names(GS$sum) == names(data)[i])]]	# Canonical cluster summary for this SNP
