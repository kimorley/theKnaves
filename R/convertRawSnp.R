# Converts raw intensity and call/confidence files to R and theta values for single SNP
convertRawSnp <- function(SNP,intuFile,gtuFile,splitIDs=TRUE){
	# Read in intensity and genotype files from GAPI
	gapiInt <- readSnpIntu(SNP,intuFile)
	gapiGtu <- readSnpGtu(SNP,gtuFile)
